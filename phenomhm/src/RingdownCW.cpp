...
{ /* Overtone Number*/
    ...
    if (2 == l && 2 == m && 0 == n)
    {
        ...
        ans = 1.0 + kappa * (1.557847 * exp(2.903124 * j) +
                             1.95097051 * exp(5.920970 * j) * kappa +
                             2.09971716 * exp(2.760585 * j) * kappa2 +
                             1.41094660 * exp(5.914340 * j) * kappa3 +
                             0.41063923 * exp(2.795235 * j) * kappa4);
    }
    else if (2 == l && 2 == m && 1 == n)
    {

        /* Fit for (l,m,n) == (2,2,1). This is a zero-damped mode in the extremal Kerr limit.*/
        ans = 1.0 + kappa * (1.870939 * exp(2.511247 * j) +
                             2.71924916 * exp(5.424999 * j) * kappa +
                             3.05648030 * exp(2.285698 * j) * kappa2 +
                             2.05309677 * exp(5.486202 * j) * kappa3 +
                             0.59549897 * exp(2.422525 * j) * kappa4);
    }
    else if (3 == l && 2 == m && 0 == n)
    {
        ...
        ans = 1.022464 * exp(0.004870 * j) +
              0.24731213 * exp(0.665292 * j) * kappa +
              1.70468239 * exp(3.138283 * j) * kappa2 +
              0.94604882 * exp(0.163247 * j) * kappa3 +
              1.53189884 * exp(5.703573 * j) * kappa4 +
              2.28052668 * exp(2.685231 * j) * kappa5 +
              0.92150314 * exp(5.841704 * j) * kappa6;
    }
    else if (4 == l && 4 == m && 0 == n)
    {

        /* Fit for (l,m,n) == (4,4,0). This is a zero-damped mode in the extremal Kerr limit.*/
        ans = 2.0 + kappa * (2.658908 * exp(3.002787 * j) +
                             2.97825567 * exp(6.050955 * j) * kappa +
                             3.21842350 * exp(2.877514 * j) * kappa2 +
                             2.12764967 * exp(5.989669 * j) * kappa3 +
                             0.60338186 * exp(2.830031 * j) * kappa4);
    }
    else if (2 == l && 1 == m && 0 == n)
    {
        ...
        ans = 0.589113 * exp(0.043525 * j) +
              0.18896353 * exp(2.289868 * j) * kappa +
              1.15012965 * exp(5.810057 * j) * kappa2 +
              6.04585476 * exp(2.741967 * j) * kappa3 +
              11.12627777 * exp(5.844130 * j) * kappa4 +
              9.34711461 * exp(2.669372 * j) * kappa5 +
              3.03838318 * exp(5.791518 * j) * kappa6;
    }
    else if (3 == l && 3 == m && 0 == n)
    {

        /* Fit for (l,m,n) == (3,3,0). This is a zero-damped mode in the extremal Kerr limit.*/
        ans = 1.5 + kappa * (2.095657 * exp(2.964973 * j) +
                             2.46964352 * exp(5.996734 * j) * kappa +
                             2.66552551 * exp(2.817591 * j) * kappa2 +
                             1.75836443 * exp(5.932693 * j) * kappa3 +
                             0.49905688 * exp(2.781658 * j) * kappa4);
    }
    else if (3 == l && 3 == m && 1 == n)
    {

        /* Fit for (l,m,n) == (3,3,1). This is a zero-damped mode in the extremal Kerr limit.*/
        ans = 1.5 + kappa * (2.339070 * exp(2.649692 * j) +
                             3.13988786 * exp(5.552467 * j) * kappa +
                             3.59156756 * exp(2.347192 * j) * kappa2 +
                             2.44895997 * exp(5.443504 * j) * kappa3 +
                             0.70040804 * exp(2.283046 * j) * kappa4);
    }
    else if (4 == l && 3 == m && 0 == n)
    {

        /* Fit for (l,m,n) == (4,3,0). This is a zero-damped mode in the extremal Kerr limit.*/
        ans = 1.5 + kappa * (0.205046 * exp(0.595328 * j) +
                             3.10333396 * exp(3.016200 * j) * kappa +
                             4.23612166 * exp(6.038842 * j) * kappa2 +
                             3.02890198 * exp(2.826239 * j) * kappa3 +
                             0.90843949 * exp(5.915164 * j) * kappa4);
    }
    else if (5 == l && 5 == m && 0 == n)
    {

        /* Fit for (l,m,n) == (5,5,0). This is a zero-damped mode in the extremal Kerr limit. */
        ans = 2.5 + kappa * (3.240455 * exp(3.027869 * j) +
                             3.49056455 * exp(6.088814 * j) * kappa +
                             3.74704093 * exp(2.921153 * j) * kappa2 +
                             2.47252790 * exp(6.036510 * j) * kappa3 +
                             0.69936568 * exp(2.876564 * j) * kappa4);
    }
    else
    {
