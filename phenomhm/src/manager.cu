...
void PhenomHM::gen_amp_phase(double *freqs_, int current_length_,
    ...
    gpuErrchk(cudaMemcpy(d_freqs[0], freqs, nwalkers*current_length*sizeof(double), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_pHM_trans[0], pHM_trans, nwalkers*sizeof(PhenomHMStorage), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_pAmp_trans[0], pAmp_trans, nwalkers*sizeof(IMRPhenomDAmplitudeCoefficients), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_amp_prefactors_trans[0], amp_prefactors_trans, nwalkers*sizeof(AmpInsPrefactors), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_pDPreComp_all_trans[0], pDPreComp_all_trans, nwalkers*num_modes*sizeof(PhenDAmpAndPhasePreComp), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_q_all_trans[0], q_all_trans, nwalkers*num_modes*sizeof(HMPhasePreComp), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_t0[0], t0, nwalkers*sizeof(double), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_phi0[0], phi0, nwalkers*sizeof(double), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_amp0[0], amp0, nwalkers*sizeof(double), cudaMemcpyHostToDevice));

    gpuErrchk(cudaMemcpy(d_M_tot_sec[0], M_tot_sec, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    ...
    kernel_calculate_all_modes<<<gridDim, NUM_THREADS>>>(d_mode_vals[0],
          d_pHM_trans[0],
          d_freqs[0],
          d_M_tot_sec[0],
          d_pAmp_trans[0],
          d_amp_prefactors_trans[0],
          d_pDPreComp_all_trans[0],
          d_q_all_trans[0],
          d_amp0[0],
          num_modes,
          d_t0[0],
...
void PhenomHM::setup_interp_wave(){
    ...
    // fill B array
    fill_B_wave<<<waveInterpDim, NUM_THREADS>>>(d_mode_vals[0], d_B[0], current_length, num_modes*nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());

    // perform interpolation
    interp[0].prep(d_B[0], current_length, 2*num_modes*nwalkers, 1);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());

    set_spline_constants_wave<<<waveInterpDim, NUM_THREADS>>>(d_mode_vals[0], d_B[0], current_length, num_modes*nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());
...
void PhenomHM::LISAresponseFD(double* inc_, double* lam_, double* beta_, double* psi_, double* t0_epoch_, double* tRef_wave_frame_, double* tRef_sampling_frame_, double* merger_freq_){
    ...
    gpuErrchk(cudaMemcpy(d_H[0], H, 9*num_modes*nwalkers*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
    ...
    gpuErrchk(cudaMemcpy(d_inc[0], inc, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_lam[0], lam, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_beta[0], beta, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_psi[0], psi, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_phiRef[0], phiRef, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_t0_epoch[0], t0_epoch, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_tRef_wave_frame[0], tRef_wave_frame, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_tRef_sampling_frame[0], tRef_sampling_frame, nwalkers*sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_merger_freq[0], merger_freq, nwalkers*sizeof(double), cudaMemcpyHostToDevice));

    //printf("inc %lf, beta %lf, lam %lf, psi %lf, phiRef %e, t0_epoch %lf, tRef %lf\n", inc, beta, lam, psi, phiRef, t0_epoch, tRef);
    // Perform response
    kernel_JustLISAFDresponseTDI_wrap<<<gridDim, NUM_THREADS>>>(d_mode_vals[0], d_H[0], d_freqs[0], d_freqs[0], d_log10f, d_l_vals, d_m_vals,
                num_modes, current_length, d_inc[0], d_lam[0], d_beta[0], d_psi[0], d_phiRef[0], d_t0_epoch[0],
                d_tRef_wave_frame[0], d_tRef_sampling_frame[0], d_merger_freq[0], TDItag, 0, nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());
...
void PhenomHM::setup_interp_response(){
    ...
    fill_B_response<<<responseInterpDim, NUM_THREADS>>>(d_mode_vals[0], d_B[0], current_length, num_modes*nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());

    interp[0].prep(d_B[0], current_length, 8*num_modes*nwalkers, 1);  // TODO check the 8?
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());

    set_spline_constants_response<<<responseInterpDim, NUM_THREADS>>>(d_mode_vals[0], d_B[0], current_length, num_modes*nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());
...
void PhenomHM::perform_interp(){
    ...
    interpolate<<<mainInterpDim, NUM_THREADS>>>(d_template_channel1[0], d_template_channel2[0], d_template_channel3[0], d_mode_vals[0], num_modes,
        d_log10f, d_freqs[0], current_length, d_data_freqs[0], data_stream_length, d_t0_epoch[0],
        d_tRef_sampling_frame[0], d_channel1_ASDinv[0], d_channel2_ASDinv[0], d_channel3_ASDinv[0], t_obs_dur, nwalkers);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());
    ...
    //print_mem_info();
     ...
     for (int i=0; i<nwalkers; i++){
         ...
         // get data - template terms
          stat = cublasZdotc(handle[0], data_stream_length,
                  &d_template_channel1[0][data_stream_length*i], 1,
                  d_data_channel1[0], 1,
                  &result);
          ...
          stat = cublasZdotc(handle[0], data_stream_length,
                  &d_template_channel2[0][data_stream_length*i], 1,
                  d_data_channel2[0], 1,
                  &result);
          status = _cudaGetErrorEnum(stat);
           cudaDeviceSynchronize();
          ...
          stat = cublasZdotc(handle[0], data_stream_length,
                  &d_template_channel3[0][data_stream_length*i], 1,
                  d_data_channel3[0], 1,
                  &result);
          status = _cudaGetErrorEnum(stat);
           cudaDeviceSynchronize();
...
void PhenomHM::GetTDI (cmplx* channel1_, cmplx* channel2_, cmplx* channel3_) {
  ...
  assert(current_status > 4);
  gpuErrchk(cudaMemcpy(channel1_, d_template_channel1[0], data_stream_length*nwalkers*sizeof(cmplx), cudaMemcpyDeviceToHost));
  gpuErrchk(cudaMemcpy(channel2_, d_template_channel2[0], data_stream_length*nwalkers*sizeof(cmplx), cudaMemcpyDeviceToHost));
  gpuErrchk(cudaMemcpy(channel3_, d_template_channel3[0], data_stream_length*nwalkers*sizeof(cmplx), cudaMemcpyDeviceToHost));
}
