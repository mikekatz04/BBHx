...
cdef extern from "SpecialLikelihood.hh":
    ...
    void InterpTDILike(cmplx* d_h, cmplx* h_h, cmplx* dataChannels, double* psd, double* dataFreqs, double* freqs, double* propArrays, double* c1, double* c2, double* c3, double* t_start_in, double* t_end_in, int length, int data_length, int numBinAll, int numModes, long* inds_ptrs, int* inds_start, int* ind_lengths, double df, int* data_index_all, int num_data_sets, int* noise_index_all, int num_noise_sets, int gpu) except+

@pointer_adjust
def InterpTDILike_wrap(d_h, h_h, dataChannels, psd, dataFreqs, freqs, propArrays, c1, c2, c3, t_start, t_end, length, data_length, numBinAll, numModes, inds_ptrs, inds_start, ind_lengths, df, data_index_all, num_data_sets, noise_index_all, num_noise_sets, gpu):
